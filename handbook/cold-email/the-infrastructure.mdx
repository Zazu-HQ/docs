...
"og:title": "The Infrastructure | The Cold Email Handbook"
"og:image": "/images/handbook.jpg"
description: "Step-by-step deliverability setup: adjacent domains, SPF/DKIM/DMARC, inbox creation, and warming so emails reach people, not their spam folders."
---
...
In order to send at any reasonable scale, you need to buy what I call **adjacent domains**: domains that are similar to your company's. In the case of my company, Za-zu, our actual domain is za-zu.com—but adjacent domains might be:
...
<Tip>Use .com domains. Everything else (.io, .net, etc.) typically sees lower deliverability rates and trust scores. For more on [picking the best domains for cold email](/choosing-domains) &larr; read here.</Tip>

Once you buy the domains, you should 301 redirect them to your main domain. For example, if I bought za-zuhq.com, I’d redirect it to za-zu.com. You do this so that if someone who sees your email decides to copy and paste your domain into their browser (this is common), they hit your real website. You’d be surprised by how few people are surprised when domains redirect this way, or even notice it. It’s less of a problem than you probably think.

### How many adjacent domains should you buy?
The number of adjacent domains you should buy depends on how much email volume you want to send. At Za-zu and Aurora, we set up 3 inboxes per domain and send 20 cold emails per inbox per day, which is about what a normal human sales rep would send. If you already know how much outbound you want to send, you can use a simple formula:
...
If you want to send 60 or fewer emails per day, you may just need one domain to get started. If you want to send 6,000, you should buy 110 domains. This is not a hard rule, but it’s a good heuristic you can use to get started. Your experience while sending may vary, and you might change your mind down the road. But it has worked for us.
...
- <Tooltip tip ="SPF designates which mail servers are permitted to send emails from your domain. It’s why you can’t spoof your email to send from tim@apple.com—you aren’t permitted to send from that email and domain.">SPF</Tooltip>
- <Tooltip tip="DKIM is a type of authentication that confirms your email hasn’t been altered between your desk and hitting your recipient’s inbox.">DKIM</Tooltip>
- <Tooltip tip="DMARC helps email receivers understand what to do with emails based on SPF and DKIM checks. Without DMARC, email service providers are not sure how you want them to handle SPF or DKIM fails from your domain.">DMARC</Tooltip>

These three acronyms, while they look rather scary, are easy to set up and provide some of the basic foundations for work we'll do later. [Here is how to set up each.](/domain-setup) *Or if you'd rather not deal with DNS records, [Za-zu](https://app.za-zu.com) can purchase domains and configure everything for you—included with your subscription.*
...
As you might expect, this is also the trickiest part of deliverability optimization. But even though some of the next steps are tricky, they should take you no more than ~10 hours to implement, even if you have lots of email inboxes. It begins, of course, with actually setting up your email inboxes. *Or save the 10 hours—[Za-zu](https://app.za-zu.com) handles all of this for you.*

### Setting up your email inboxes
At Za-zu and my agency Aurora, we set up 3 inboxes per domain. You could set up more—it’s the temptation, because setting up lots of domains takes work—but we don’t do this. That’s because the more you send from each domain, the more spam reports there are per domain. More spam reports on a domain increases the chance that the entire domain crosses a spam threshold and gets a bad reputation for being spammy. To reduce risk, diversify sending across more domains.
...
<Step title="Temporary phone numbers">
Even with VPNs and antidetect browsers, Google may still detect what you're doing and require phone number verification for new accounts. The problem is that once you verify a phone number with Google, you can typically only use it a couple more times before they block it. This severely limits how many accounts you can create. The solution is to use a service like [TextVerified](https://textverified.com/) that lets you rent real phone numbers for just a single text verification. This way, each time Google asks for phone verification, you can use a fresh number and continue creating the additional Google accounts you need for your email infrastructure.
</Step>
</Steps>

<Note>Look, this part is tedious. VPNs, antidetect browsers, temporary phone numbers—it's the kind of work that eats up hours and feels pretty far from actually reaching customers. If you'd rather skip it, [Za-zu](https://app.za-zu.com) handles all of this (buying domains, DNS setup, inbox creation, the whole thing) as part of your subscription. Or keep reading and we'll walk you through it.</Note>

Ok so you use these 3 workarounds, or have Za-zu automatically do it all for you. Then set up your inboxes with the names of real humans that work on your team. At Aurora, we found it’s usually most effective to use the names of the founders or other people that can open doors at the company (e.g. your Head of [Department] type roles). Prospects often respond better to this than getting an email from a low-level salesperson. It’s fine if the low-level salesperson is managing outbound on behalf of the founder, but I find it’s effective to use the name of people who are higher up. This isn’t one-size-fits-all advice, though—think about what makes sense for your company. And, yes, use profile pictures!
...
If you aren’t interested in learning more of the workings of email warming, [skip to the next section](/handbook/cold-email/writing-great-emails) where we cover evidence-backed methods for writing a great cold email.
...
Speaking of making sales... next chapter starts here → [Sending Personalized Outbound at Scale
](/handbook/cold-email/personalizing-at-scale)

<Frame>
<div class="flex-col items-center my-2 px-2">
<div class="font-semibold">Enjoying this playbook so far?</div>
<div className='mb-2'>Just write your email in the box below to sign up to our newsletter. You'll get an email every time we publish a new piece. No spam, just cold email tactics.</div>
<Newsletter />
</div>
</Frame>
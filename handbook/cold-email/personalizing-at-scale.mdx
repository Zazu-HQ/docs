...
"og:title": "Sending Personalized Outbound at Scale | The Cold Email Handbook"
description: "Use AI to personalize cold emails and run scalable campaigns—templates, prompting, word shuffling, and optimization."
...
Or you could try the product I have been building over the last year: Za-zu. It's the email sequencer built for personalized outbound at scale, and it can do everything from warming your accounts to implementing all of the smart AI tactics we'll discuss below. [See the product here](https://app.za-zu.com).
...
><br/>
> If so - you should check out Za-zu, my company. It's an email sequencer that handles deliverability automatically. Most cold emails vanish into spam. We make sure yours land in Primary.
><br/>
...
- **Copy and paste**: The <VariableNode>first_name</VariableNode> variable in this email is the simplest one—you just need to paste the name of the prospect.

- **Summarization and normalization**: The <VariableNode>company_name</VariableNode> and <VariableNode>startup_category</VariableNode> variables go beyond the simple copy and paste. The company name, for example, needs to be normalized—if a company's name is 'Hypothetical Company Industries, Inc.', pasting that entire thing in an email will feel inhuman. So we want AI to shorten it to 'Hypothetical Company' or even just 'Hypothetical' if that's what people would casually call them. And <VariableNode>startup_category</VariableNode> needs to be a human-sounding summary of the specific niche or industry that the prospect's company is in.

- **If-then:** The investor line at the beginning of the email feels tricky, but it isn't. You just need to ask Za-zu to cross-reference the list of the prospect's investors with your own lists of previous investors (relevant in this case, since I have them). If there's an investor in common, the line will be included—if not, it will be cut.

AI personalization can, of course, go as far as you want it to. It can write entire sentences or paragraphs based on the instructions you give it. We'll talk through how to do this kind of thing in the next section. At Aurora, for example, one campaign for a podcasting agency that found success included a bullet-point section where AI offered personalized podcast ideas for the prospects. Always write your dream email first, then figure out how to replicate that email with AI at scale second.
...
## How to use AI at scale with spreadsheets

Chatting with ChatGPT or Claude allows you to have one conversation at a time. But let's say for each prospect, you have 3 different AI variables to create. That's 3 conversations each, or across 1000 leads, 3000 different conversations. That would take days or weeks on chatgpt.com. But there's a way that's a million times faster.

Allow me to introduce you (well, re-introduce you) to the spreadsheet.

Whether you use Google Sheets ([requires a free extension to use AI](https://workspace.google.com/marketplace/app/gpt_for_sheets_and_docs/677318054654)) or premium tools with built-in AI like Rows.com (starts at \$80/m) or Clay.com (starts at \$150/m), the concept is the same: run your prompt(s) once across hundreds of rows simultaneously.

Here's how it works: set up your prompt, test it on a few rows to make sure the output is clean, then run it across your entire lead list. Ten minutes later, you have personalized variables for hundreds of prospects based on whatever data you've pulled—company descriptions, LinkedIn bios, recent news, whatever.

<img
  className=""
  src="/images/spreadsheet-ai-example.png"
  alt="A spreadsheet that is enriching each row with an AI column"
/>

Most email sequencers (including Za-zu) let you import spreadsheets directly, so once your AI variables are ready, you're good to go. The spreadsheet becomes your personalization factory.
...
>Imagine you're the CMO at <VariableNode>company_name</VariableNode>. Your task is to succinctly fill in the blank in the sentence 'I thought a show about ______ could be impactful' with an idea for what their podcast show could be about.<br/>
> The show should aim to get the company "out there" and help them grow. Let the company description be your guide.
><br/>
>The company description: "<VariableNode>company_description</VariableNode>"
><br/>

<Tip>When I'm writing an AI prompt, I use what I call the *Three C's*: Context, Creative, and Constraint. What you just read above was the *Context*. Now I'm moving into the *Creative Constraint*—telling it how to be creative and where the guardrails are.</Tip>

>- Your output should be just the part that NATURALLY fills in the blank, not the whole sentence. In other words output only the segment that fills in the blank, nothing else.
>- Choose only key terms that are essential for identification, ensuring your output integrates smoothly into the template sentence without grammatical adjustments. 
>- Aim for a natural, conversational tone. If the idea requires a preposition or article for a more fluent fit, include it. The goal is to create an output that can be directly inserted into the sentence, maintaining clarity and simplicity.
>- Keep it brief: 1-5 words, no more. Aim for punchy and to-the-point.<br/>
>- Maintain a casual yet polished tone, like you're brainstorming in a relaxed, high-level meeting.<br/>
> - Prioritize lowercase usage, even for typically capitalized terms. Abbreviate where it makes sense and enhances clarity.<br/>
> - Steer clear of complex jargon; opt for simplicity and impact.<br/>
>- Leave out sentence-ending punctuation in your output.
...
<Tip> These creative constraints didn't come out of thin air—I built them iteratively. I started with a few basic rules, ran the prompt, then refined it based on what worked and what didn't. It takes some upfront work, but once you've dialed it in, you'll have a system that delivers what you want, every time.</Tip>
...
> You are an expert at identifying what niche a company operates in. Your job is to read a company's LinkedIn description and distill their primary niche into a clear phrase.
Focus on keywords that reveal the company's core business, unique activities, or main product. Be direct and precise. No fluff or corporate speak.
...
- Between Stage 1 and 2: **3 days**
- Between Stage 2 and 3: **7 days**
...
Simple enough, and any email sequencer built for outbound at scale (like Za-zu) will either automatically do this or at least make it possible to shuffle words with some tinkering.
...
This is a tactic commonly used by copywriters. Write dozens of different angles, focus on the ones that work, then write more variants in the same direction of your best angles. It’s good advice for copywriting and equally good advice for sending effective outbound.
...
The Last Chapter → [Launching Your Campaigns](/handbook/cold-email/launching)
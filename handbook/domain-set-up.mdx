...
**SPF** stands for *Sender Policy Framework*. It designates which mail servers are permitted to send emails from your domain. It's why you can't spoof your email to send from `tim@apple.com`—you aren't permitted to send from that email and domain.
...
**3. Validate that the SPF record is set up correctly** using [Fraudmarc's SPF lookup](https://fraudmarc.com/tools/spf/lookup).
...
Once your DKIM record is published, verify it using DKIM record checking tools like [MXToolbox](https://mxtoolbox.com/dkim.aspx) to ensure it's valid and correctly formatted.
...
**2. Validate the DMARC record is set up correctly** using [Fraudmarc's DMARC lookup](https://fraudmarc.com/tools/dmarc/lookup).

...
description: "Smartlead and Instantly work, but they're slow, clunky, and force you to write code. I built Za-zu because I got so frustrated with them. Here's why Za-zu is cleaner, simpler, and does the hard work automatically—so you don't have to become a deliverability expert."
---
...
Here's what it looks like to build an <Tooltip tip="Say one thing if a conditional statement is true, and something else if it's false">if/then statement</Tooltip> in za-zu.
...
And here's what it looks like to add <Tooltip tip="A smart way to automatically vary the wording in each email to maximize deliverability. Just add synonym alternatives inside your text, and Za-zu randomly picks a different one every time your message sends.">word shuffling</Tooltip> to a campaign in za-zu.
<img
  className=""
  src="/images/word-shuffling.gif"
  alt="Sending the exact same email hundreds (or thousands) of times triggers alarm bells for ESPs like Google. Fair enough, normal humans rarely type identical emails on repeat. Enter Word Shuffling: a smart way to automatically vary the wording in each email. Just add synonym alternatives inside your text, and Za-zu randomly picks a different one every time your message sends."
/>

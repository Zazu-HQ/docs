...
"og:title": "Domain Setup for Cold Email: SPF, DKIM, DMARC Guide"
sidebarTitle: "Domain Setup"
"og:description": "Domain setup is the foundation of cold email deliverability. SPF, DKIM, and DMARC aren't optional—they're what separates emails that land in inboxes from those that disappear into spam. Here's how to set them up in under an hour."
---
...
If you add another service that sends **on your behalf** (i.e. not through Gmail), like Mailgun or Postmark, then you’d update your SPF to include them:
...
Once your DKIM record is published, verify it using DKIM record checking tools like MXToolbox to ensure it's valid and correctly formatted.

...
**Validate email lists to check whether emails will bounce.**

One way you can send a major red flag to ESPs is by sending a lot of emails that bounce (in other words, that don’t reach an inbox). Anything above 3% is a major signal for spam. So, before you send out emails to a big list, verify which emails aren’t valid (i.e. bounce) and purge them from your list. There are hundreds of cheap tools that can help with this. (fyi real soon we're going to integrate this directly in za-zu so you can skip the third-party tool juggling act)
